...
def run(conn):
    ...
    # ── M002: indexes on sales_followup (skip if table no longer exists) ──────────
    if _table_exists(cur, 'sales_followup'):
        ...
        print("⏭  M002: sales_followup gone — indexes skipped")
    ...
    if not _col_exists(cur, 'customers', 'current_stage'):
        ...
        if _table_exists(cur, 'sales_followup'):
            ...
            """)
        print("✅ M003: current_stage column added")
    else:
        print("⏭  M003: current_stage already exists")

    # ── M004: extend sales_followup (skip if table gone) ────────────────────────
    if _table_exists(cur, 'sales_followup'):
        ...
        print("⏭  M004: sales_followup gone — column extensions skipped")
